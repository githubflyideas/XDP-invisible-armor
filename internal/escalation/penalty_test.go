...
func TestEscalationSequence(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		got = append(got, p.RegisterBan(true))
	}
	for i := range want {
		if got[i] != want[i] {
			t.Fatalf("阶梯[%d] 期望 %d 得 %d", i, want[i], got[i])
		}
	}
	if !p.Permanent() {
		t.Fatal("末级应永久")
...
func TestStillAttacking(t *testing.T) {
	p := NewPenalty("1.1.1.4")
	p.BaselinePackets = 100
	if p.StillAttacking(103) {
		t.Fatal("增量3不算")
	}
	if !p.StillAttacking(200) {
		t.Fatal("增量100算")
	}
}
