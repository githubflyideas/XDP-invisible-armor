...
# 单二进制:xdp-ban(控制面 + 执行器,吸收了原 xdp-agent)
.PHONY: all
all: bpf build

## bpf: 编译 eBPF 目标文件并放到 embed 目录
#
# 发布前必须先跑这个:go:embed 嵌入的是这个 .o,
# 仓库里的是 0 字节占位文件(让无 clang 的机器也能编译 Go 部分)。
# 忘了跑的话,产出的二进制内嵌空 bytecode,启动即报错。
# release 目标会强制检查这一点。
.PHONY: bpf
bpf:
	@command -v $(CLANG) >/dev/null || \
	  { echo "缺少 $(CLANG)。安装: apt-get install clang libbpf-dev"; exit 1; }
	$(CLANG) $(BPF_CFLAGS) -c bpf/xdp_filter.c -o cmd/xdpban/obj/xdp_filter.o
	@echo ">> eBPF 目标文件:"
	@ls -l cmd/xdpban/obj/xdp_filter.o

## bpf-check: 断言待嵌入的 .o 非空(防止发布出内嵌空 bytecode 的二进制)
.PHONY: bpf-check
bpf-check:
	@for f in cmd/xdpban/obj/xdp_filter.o; do \
	  if [ ! -s "$$f" ]; then \
...
dist: bpf-check
	...
	@for arch in amd64 arm64; do \
	  ...
	  CGO_ENABLED=0 GOOS=linux GOARCH=$$arch \
	    ...
	    -o $(DIST)/xdp-ban-linux-$$arch ./cmd/xdpban; \
	done
...
## fuzz: 对信任边界的解析函数跑 30s 模糊测试
.PHONY: fuzz
fuzz:
	go test -run='^$$' -fuzz='^FuzzParseIPv4Prefix$$' -fuzztime=30s ./cmd/xdpban/

## prof: 采集控制面热路径(SQLite 写审计 / 仪表板计数)的 CPU/内存 profile
.PHONY: prof
prof:
	@mkdir -p $(DIST)/prof
	go test -bench='BenchmarkWriteAudit|BenchmarkDashboardCounts' -run='^$$' \
	  -cpuprofile=$(DIST)/prof/cpu.out \
	  -memprofile=$(DIST)/prof/mem.out ./internal/model/
...
.PHONY: clean
clean:
	rm -f xdp-ban coverage.out coverage.html *.db
	rm -rf $(DIST)
	rm -f cmd/xdpban/obj/*.o
