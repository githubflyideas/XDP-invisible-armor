...
if have bpftool; then
    log "--- eBPF Map 概况 ---"
    bpftool map show 2>/dev/null | grep -iE "ban_list|counters" \
...
if have tcpreplay; then
    log "--- 流量回放提示 ---"
    log "  验证 DROP 命中率:"
    log "    sudo tcpreplay -i <业务网卡> -t -K attack.pcap"
    log "  然后核对 counters map 的 dropped 计数是否符合预期"
    log ""
fi
